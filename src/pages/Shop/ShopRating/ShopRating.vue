    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <span class="block positive" :class='{active: selectType === 2}' @click='ratingType(2)'>
            全部<span class="count">{{ratings.length}}</span>
          </span>
          <span class="block positive" :class='{active: selectType === 0}' @click='ratingType(0)'>
            满意<span class="count">{{positiveSize}}</span>
          </span>
          <span class="block negative" :class='{active: selectType === 1}' @click='ratingType(1)'>
      ...
      filterRatings () {
        /*
          条件一
              ...
              selecttype  0(满意) 1(不满意) 2(all)
              rating.rateType 0(满意) 1(不满意)

              selecttype===2 || selecttype === rateType
          条件二
              onlyShowText true(仅显示有文本的) false(全显示)
              rating.text 有值或者没有
              !onlyShowText(全选) || rating.text.length>0 (只显示有文本的)

         */
         /*
            当selectype = 2 && ！onlyshowText --> 返回整个数组
              满足它们两个条件 返回true -- 所以 全部返回
            当selectType = ratatype时 返回 满足ratetype的 条件的内容
              返回的是满足条件的过滤后的数组
